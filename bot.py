...
class MMBump:
    ...
    def handle_farming(self, telegram_id):
        auth_payload = f"telegram_id={telegram_id}"

        try:
            auth_response = self.http_request(
                self.auth_url, self.headers, data=auth_payload
            )
            if auth_response.status_code == 200:
                hash_token = self.extract_hash(auth_response)
                farming_headers = {**self.headers, "Authorization": hash_token}
    ...
    def main(self):
        ...
        while True:
            for no, telegram_id in enumerate(self.telegram_ids):
                print(self.line)
                self.log(
                    f"{blue}Account number: {white}{no + 1}/{len(self.telegram_ids)}"
                )
                self.handle_farming(telegram_id.strip())
